...
async function main() {
  ...
  const allowedIssueNumbers = holdoutSplit
    ...
    : null;
  const issues = loadIssues().filter((issue) =>
    allowedIssueNumbers ? allowedIssueNumbers.has(issue.number) : true,
  );
  const provider = createEmbeddingProvider();
  const checkpointName = args.holdout
    ? `${CHECKPOINT_NAME}.${args.holdout}`
    : CHECKPOINT_NAME;
  const checkpoint = args.reset
    ? null
    : loadCheckpoint<EmbeddingCheckpoint>(checkpointName);
  const existingRecords = args.reset ? [] : loadExistingIndex();
  const recordMap = new Map<number, IssueEmbeddingRecord>(
    existingRecords
      .filter((record) =>
        allowedIssueNumbers
          ? allowedIssueNumbers.has(record.issueNumber)
          : true,
      )
  ...
  const remainingIssues = issues.filter(
    (issue) => !completedIssueNumbers.has(issue.number),
  );
  ...
  for (let index = 0; index < batches.length; index += 1) {
    ...
    try {
      ...
      saveCheckpoint(checkpointName, {
        completedIssueNumbers: Array.from(completedIssueNumbers.values()).sort(
          (a, b) => a - b,
        ),
