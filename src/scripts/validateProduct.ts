...
import { createEmbeddingProvider, cosineSimilarity, loadEmbeddingIndex } from "../lib/embeddings.js";
...
import type { BenchmarkCase, HoldoutSplit, QueryIssueResult, RecommendationResult } from "../lib/types.js";

const HOLDOUT_PATH = path.resolve(process.cwd(), "data", "processed", "evaluation", "holdout.json");
const OUTPUT_PATH = path.resolve(process.cwd(), "data", "processed", "evaluation", "productValidation.json");
...
function aggregateRecommendation(
  ...
  evidenceRows: Array<{ candidate: BaselineCandidate; evidence: QueryIssueResult }>
): RecommendationResult {
  const componentScores = new Map<string, number>();
  const fileScores = new Map<string, { file_path: string; component: string; reason: string; score: number }>();
  ...
  evidenceRows.forEach((result, index) => {
    ...
    for (const file of uniqueFiles) {
      componentScores.set(file.component, (componentScores.get(file.component) || 0) + issueWeight);
  ...
  const evidencePath = evidenceRows.flatMap((row) => row.evidence.evidencePaths).slice(0, 10);
...
async function generateLexicalGraphRecommendation(
  ...
  const queryText = [benchmarkCase.title, benchmarkCase.body].filter(Boolean).join("\n\n");
  const issueNumbers = await searchIssuesByTitle(queryText, Math.max(topK * 6, 24));
  const evidenceRows = (
    await Promise.all(
    ...
    .filter((row): row is { issueNumber: number; evidence: QueryIssueResult } => row.evidence !== null)
    ...
    .map((row) => ({
      candidate: {
        ...
        score: keywordScore(queryText, `${row.evidence.issue.title}\n${row.evidence.issue.labels.join(" ")}`)
      },
      evidence: row.evidence
    }))
    .sort((left, right) => right.candidate.score - left.candidate.score || left.candidate.issueNumber - right.candidate.issueNumber)
    .slice(0, topK);
...
async function generateSemanticGraphRecommendation(
  ...
  const rankedCandidates = embeddingIndex
    ...
    .sort((left, right) => right.score - left.score || left.issueNumber - right.issueNumber)
  ...
  const evidenceRows = (
    ...
    .filter((row): row is { candidate: BaselineCandidate; evidence: QueryIssueResult } => row.evidence !== null)
    .filter((row) => row.evidence.evidencePaths.length > 0)
    .slice(0, topK);
...
async function main() {
  ...
  const systems: SystemName[] = ["lexical_graph", "semantic_graph", "full_graph_aware"];
  const accumulators = Object.fromEntries(systems.map((name) => [name, initializeMetrics()])) as Record<
    SystemName,
    SystemMetrics
  >;
  ...
  for (const benchmarkCase of cases) {
    ...
    const outputs: Record<SystemName, RecommendationResult> = {
      ...
      semantic_graph: await generateSemanticGraphRecommendation(benchmarkCase, embeddingIndex),
    ...
    const caseRow: ValidationCaseRow = {
      ...
      outcomes: {
        lexical_graph: {
          ...
          duplicate_issue: outputs.lexical_graph.possible_duplicate.issue_number,
          top_files: outputs.lexical_graph.likely_impacted_files.slice(0, 5).map((file) => file.file_path),
          top_similar_issues: outputs.lexical_graph.similar_tickets.slice(0, 3).map((ticket) => ticket.issue_number),
        ...
        semantic_graph: {
          ...
          duplicate_issue: outputs.semantic_graph.possible_duplicate.issue_number,
          top_files: outputs.semantic_graph.likely_impacted_files.slice(0, 5).map((file) => file.file_path),
          top_similar_issues: outputs.semantic_graph.similar_tickets.slice(0, 3).map((ticket) => ticket.issue_number),
          focused_file_hit: false,
          component_correct: false
        },
        full_graph_aware: {
          ...
          duplicate_issue: outputs.full_graph_aware.possible_duplicate.issue_number,
          top_files: outputs.full_graph_aware.likely_impacted_files.slice(0, 5).map((file) => file.file_path),
          top_similar_issues: outputs.full_graph_aware.similar_tickets.slice(0, 3).map((ticket) => ticket.issue_number),
          focused_file_hit: false,
          component_correct: false
        }
      }
    ...
    for (const system of systems) {
      ...
      const topFilesAt5 = recommendation.likely_impacted_files.slice(0, 5).map((file) => file.file_path);
      ...
      const componentCorrect = recommendation.suggested_component === benchmarkCase.expected.component;
      const componentTop3Correct =
        componentCorrect || top3Components.includes(benchmarkCase.expected.component);
      ...
      const evidencePathValid =
        ...
        recommendation.evidence_path.every((pathRow) => isEvidencePathValid(pathRow));

      accumulators[system].evaluated_cases += 1;
      accumulators[system].component_accuracy += componentCorrect ? 1 : 0;
      accumulators[system].component_top3_accuracy += componentTop3Correct ? 1 : 0;
      accumulators[system].duplicate_support_rate += recommendation.similar_tickets.length > 0 ? 1 : 0;
      accumulators[system].component_aligned_file_hit_rate += componentAlignedHit ? 1 : 0;
      accumulators[system].focused_file_recall_at_5 += focusedRecallAt5;
      accumulators[system].evidence_path_validity_rate += evidencePathValid ? 1 : 0;
    ...
    if (!fullGraphOutcome.component_correct || !fullGraphOutcome.focused_file_hit) {
