...
export function loadSemanticEnv(): SemanticEnv {
  const provider = (process.env.EMBEDDING_PROVIDER?.trim().toLowerCase() || "local") as
    | "local"
    | "gemini";
  ...
  const dimension = parseNumberEnv("EMBEDDING_DIMENSION", provider === "gemini" ? 768 : 256);
  ...
  return {
    ...
    EMBEDDING_MODEL: process.env.EMBEDDING_MODEL?.trim() || "gemini-embedding-001",
    EMBEDDING_DIMENSION: dimension,
    EMBEDDINGS_FILE_PATH:
      process.env.EMBEDDINGS_FILE_PATH?.trim() || "data/processed/embeddings/issues.embeddings.json",
